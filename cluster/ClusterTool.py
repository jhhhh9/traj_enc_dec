...
# todo 参数改变
def DoKMeansWithError(all_label, k=10, feature=None, save=False, epoch="-1"):
    '''
    ...
    inertia_end = calculate_feature_center_dist(data, centroids)
    ...
    #     save_cluster_h5(file_original_path, file_cluster_path, labels)
        ...
        DoTSNE(data, 2, k, labels_dict, ori_labels_dict, centroids, 'png_dtc_tsne/',epoch=epoch)

    return centroids, error_total, nmi, ari, inertia_start, inertia_end, n_iter, labels
    # return centroids, score, inertia_start, inertia_end, n_iter
...
def DoDBSCANWithError(feature_path, file_original_path, k=19, n=1899, center=None, feature=None, save=False,epoch=-1):
    ...
    # -----3、kmeans聚类
    # dbscan = DBSCAN(eps=0.95).fit(data)
    ...
    nmi = calculate_nmi(list(labels_dict.values()), list(ori_labels_dict.values()))
    acc = calculate_acc(list(labels_dict.values()), list(ori_labels_dict.values()))

    # -----4、绘制聚类结果并保存
    if save == True:
        save_png(file_original_path, 'cluster/png_new/', k, labels, epoch=epoch,error=error_tri)
        # DoTSNE(data, 2, k, labels_dict, centroids, 'cluster/png_tsne/',epoch=epoch)
    centroids=None
    inertia_start=None
    inertia_end=None
    n_iter=None
...
def DoTSNE(features, n_components, k, labels_dict, ori_labels_dict, centers,file_png,epoch="-1"):
    ...
    data_embedded = TSNE(n_components=n_components).fit_transform(data)
    ...
    colors = ['red', 'green', 'yellow', 'c', 'darkred', 'lightcoral', 'tan', 'blue', 'orange','lightgreen',
              'brown', 'wheat', 'navy','pink',
              'springgreen',  'turquoise', 'slateblue', 'teal', 'plum', 'skyblue', 'hotpink',
              ]
    ...
    for i in range(k):
        ...
        # plt.savefig(os.path.join(file_png, '%d-%d.png' % (e,i)))
        # plt.cla()
        # plt.clf()
        # plt.close()
    ...
    plt.axis('off')
    plt.savefig(os.path.join(file_png, 'epoch-'+epoch+'.png'), bbox_inches='tight')
    plt.cla()
    plt.clf()
    plt.close()
